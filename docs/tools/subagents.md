...
- Sub-agent context only injects `AGENTS.md` and `TOOLS.md` (no `SOUL.md`, `IDENTITY.md`, `USER.md`, `MEMORY.md`, `HEARTBEAT.md`, or `BOOTSTRAP.md`). Codex-native subagents follow the same boundary: parent turns receive `SOUL.md`, `IDENTITY.md`, `USER.md`, and `TOOLS.md` as turn-scoped collaboration instructions in workspace order, while a child-only inherited `TOOLS.md` block remains in Codex thread instructions so native Codex subagents still receive delegated-worker tool context.
